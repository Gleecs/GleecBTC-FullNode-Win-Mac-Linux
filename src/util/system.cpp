...
const char * const GLEECBTC_CONF_FILENAME = "gleecbtc.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "gleecbtcd -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //gleecbtc-tx using stdin
        std::string val;
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", GLEECBTC_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure GleecBTC Core copyright is not removed by accident
    if (copyright_devs.find("GleecBTC Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The GleecBTC Core developers";
    }
