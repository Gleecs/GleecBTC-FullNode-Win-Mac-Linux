This directory contains the GleecBTCQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the GleecBTC UI.
...
ClientModel is used by the main application `gleecbtcgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/gleecbtc/gleecbtc/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a gleecbtc:... link from a browser).
...
* UI elements like GleecBTCAmountField, which inherit from QWidget.
* `gleecbtcstrings.cpp`: automatically generated
* `gleecbtcunits.(h/cpp)`: GLEEC / mGLEEC / etc handling
* `callback.h`
...
* don't change `local/gleecbtc_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "gleecbtc-qt" under "Run", which is where you can also add command line arguments)
