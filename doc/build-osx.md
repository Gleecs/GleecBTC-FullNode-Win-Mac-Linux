...
1. Clone the GleecBTC Core source code:
    ```shell
...
2.  Build GleecBTC Core:
    ...
    Configure and build the headless GleecBTC Core binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, GleecBTC Core may be
...
```shell
mkdir -p "/Users/${USER}/Library/Application Support/GleecBTC"

touch "/Users/${USER}/Library/Application Support/GleecBTC/gleecbtc.conf"

chmod 600 "/Users/${USER}/Library/Application Support/GleecBTC/gleecbtc.conf"
```

The first time you run gleecbtcd, it will start downloading the blockchain. This process could
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/gleecbtc-core/gleecbtc-detached-sigs).
