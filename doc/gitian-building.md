...
*Setup instructions for a Gitian build of GleecBTC Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GleecBTC
...
- [Building GleecBTC Core](#building-gleecbtc-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-gleecbtc-host/README.md).
...
Clone the git repositories for gleecbtc and Gitian.
...
in the gleecbtc repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building GleecBTC Core
----------------

To build GleecBTC Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the gleecbtc repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gleecbtc/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/gleecbtc/gleecbtc
...
For example:
```bash
URL=https://github.com/laanwj/gleecbtc.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit gleecbtc=${COMMIT} --url gleecbtc=${URL} ../gleecbtc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gleecbtc=${COMMIT} --url gleecbtc=${URL} ../gleecbtc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gleecbtc=${COMMIT} --url gleecbtc=${URL} ../gleecbtc/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the gleecbtc git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../gleecbtc/contrib/gitian-descriptors/*|sort|uniq )
...
GBCPATH=/some/root/path/gleecbtc
SIGPATH=/some/root/path/gleecbtc-detached-sigs

./bin/gbuild --url gleecbtc=${GBCPATH},signature=${SIGPATH} ../gleecbtc/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gleecbtc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gleecbtc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/gleecbtc-osx-build.assert
```
...
[gleecbtc-core/gitian.sigs](https://github.com/gleecbtc-core/gitian.sigs/) repository, or if that's not possible create a pull
