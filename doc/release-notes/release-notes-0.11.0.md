GleecBTC Core version 0.11.0 is now available from:

  <https://gleecbtc.org/bin/gleecbtc-core-0.11.0/>
...
For example, add the following to `gleecbtc.conf`:
...
related to the blockchain in the gleecbtc system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows GleecBTC Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that GleecBTC Core will 
...
For an example script see [gleecbtc-submittx](https://github.com/laanwj/gleecbtc-submittx).
...
- #5334 `cf87536` libgleecbtcconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new gleecbtc-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only GleecBTCAmountField
- #5753 `0f44672` Add gleecbtc logo to about screen
...
- #5809 `46bfbe7` Add gleecbtc-cli man page
...
- #5849 `41259ca` contrib/init/gleecbtcd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename GleecBTC-Qt.app to GleecBTC-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for gleecbtc-tx)
