# RapidCheck property-based testing for GleecBTC Core

## Concept

Property-based testing is experimentally being added to GleecBTC Core with
...
If RapidCheck is installed, GleecBTC Core will automatically run the
...
3. Configure GleecBTC Core with RapidCheck.

    `cd` to the directory of your local gleecbtc repository and run
...
4. Build GleecBTC Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_gleecbtc-crypto_gen.o
    CXX      test/test_gleecbtc-key_properties.o
    ```
...
That's it! You are now running property-based tests in GleecBTC Core.
