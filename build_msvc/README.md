Building GleecBTC Core with Visual Studio
========================================
...
Quick Start
---------------------
The minimal steps required to build GleecBTC Core with the msbuild toolchain are below. More detailed instructions are contained in the following sections.
...
msbuild /m build_msvc\gleecbtc.sln /p:Platform=x64 /p:Configuration=Release /t:build
```

Dependencies
---------------------
A number of [open source libraries](https://github.com/gleecbtc/gleecbtc/blob/master/doc/dependencies.md) are required in order to be able to build GleecBTC Core.
...
The [external dependencies](https://github.com/gleecbtc/gleecbtc/blob/master/doc/dependencies.md) required for building are:
...
Qt
---------------------
All the GleecBTC Core applications are configured to build with static linking. In order to build the GleecBTC Core Qt applications a static build of Qt is required.

The runtime library version (e.g. v141, v142) and platform type (x86 or x64) must also match. OpenSSL must also be linked into the Qt binaries in order to provide full functionality of the GleecBTC Core Qt programs. An example of the configure command to build Qtv5.9.7 locally to link with GleecBTC Core is shown below (adjust paths accordingly), note it can be expected that the configure and subsequent build will fail numerous times until dependency issues are resolved.
...
A prebuilt version for x64 and Visual C++ runtime v141 (Visual Studio 2017) can be downloaded from [here](https://github.com/sipsorcery/qt_win_binary/releases). Please be aware this download is NOT an officially sanctioned GleecBTC Core distribution and is provided for developer convenience. It should NOT be used for builds that will be used in a production environment or with real funds.

To build GleecBTC Core without Qt unload or disable the gleecbtc-qt, libgleecbtc_qt and test_gleecbtc-qt projects.
...
AppVeyor
---------------------
The .appveyor.yml in the root directory is suitable to perform builds on [AppVeyor](https://www.appveyor.com/) Continuous Integration servers. The simplest way to perform an AppVeyor build is to fork GleecBTC Core and then configure a new AppVeyor Project pointing to the forked repository.

For safety reasons the GleecBTC Core .appveyor.yml file has the artifact options disabled. The build will be performed but no executable files will be available. To enable artifacts on a forked repository uncomment the lines shown below:

```
    #- 7z a gleecbtc-%APPVEYOR_BUILD_VERSION%.zip %APPVEYOR_BUILD_FOLDER%\build_msvc\%platform%\%configuration%\*.exe
    #- path: gleecbtc-%APPVEYOR_BUILD_VERSION%.zip
```
