...
The RPM spec file provided here is for GleecBTC-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://gleecbtc.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[GleecBTC git repository](https://github.com/gleecbtc/gleecbtc) but are not part of
...
Currently only `Source10` is used, for the example `gleecbtc.conf` file.
...
in the gleecbtc git repository. At present only an SVG version of the GleecBTC
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the gleecbtc
...
needed when 0.12.1 is released, a proper fix is already in the GleecBTC git
master branch.
...
idea to build GleecBTC Core against a static version of that library compiled
...
The `gleecbtc.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `gleecbtc-server` package. At this point, most RPM based
...
The `gleecbtc-server` package should have SELinux support. How to properly do
...
GleecBTC-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/gleecbtc/). He has been
packaging GleecBTC for Fedora at least since 2012.
...
4. I renamed the `gleecbtc` package that contains the Qt GUI to `gleecbtc-core` as
that appears to be how the general population refers to it, in contrast to
`gleecbtc-xt` or `gleecbtc-classic`. I wanted to make sure the general population
